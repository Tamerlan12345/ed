...
`;
            ...
            finalPrompt = `
                ...
                1.  **ПРИОРИТЕТ ДАННЫХ (ВЫСШИЙ ПРИОРИТЕТ):**
                    *   **Поиск цифр:** Твоя главная задача — найти и отобразить страховые премии, страховые суммы, тарифы и таблицы тарифов.
                    *   **ЗАПРЕТ НА СКРЫТИЕ:** Если эти данные есть в тексте, ты **ОБЯЗАН** включить их в слайды. Категорически ЗАПРЕЩЕНО писать "см. в приложении" или "подробнее в договоре", если цифры физически присутствуют в исходном тексте.
                    *   **Ссылка на приложение:** Разрешено отправлять пользователя к документам ("см. Приложение") **ТОЛЬКО** в том случае, если в исходном тексте **НЕТ** конкретных цифр или таблиц.

                2.  **ПРАВИЛО СОЗДАНИЯ СЛАЙДОВ (ТАБЛИЦЫ):**
                    *   Если таблица с тарифами/премиями объемная — **СОЗДАЙ НОВЫЙ СЛАЙД** (или несколько) сразу после слайда с описанием продукта.
                    *   Заголовок этого нового слайда должен быть строго: **"Страховые премии и Тарифы"**.
                    *   Для верстки цифр ОБЯЗАТЕЛЬНО используй HTML-теги \`<table>\`, \`<tr>\`, \`<td>\`, чтобы сохранить табличную структуру.

                3.  **Полнота информации:**
                    *   **Запрет сокращений:** Нельзя необоснованно сокращать информацию.
                    *   **Развернутые тезисы:** Используй полноценные предложения.

                4.  **Сноска для цифр:**
                    *   Если на слайде есть цифры (премии, тарифы), добавь сноску: \`<small>*Информация может быть не актуальна. Пожалуйста, проверяйте актуальные данные в программе страхования.</small>\`

                5.  **Структура курса (минимум 12 слайдов, можно больше если есть таблицы):**
                    *   Слайды 2-11+: Основная часть.
                    *   Последний слайд: Выводы.

                6.  **Правило переполнения (для текста, НЕ для таблиц):**
                    *   Если ТЕКСТА слишком много, выдели главное, поставь (*) и добавь сноску: \`<small>* - полные условия см. в Договоре.</small>\`
                    *   ЭТО ПРАВИЛО НЕ РАСПРОСТРАНЯЕТСЯ НА ТАБЛИЦЫ ТАРИФОВ - они должны быть полностью перенесены на новые слайды.

                7.  **Формат HTML:** Заголовки \`<h2>\`, списки \`<ul>\`, выделение \`<strong>\`, таблицы \`<table>\`.

                КЛЮЧЕВЫЕ ТРЕБОВАНИЯ К ТЕСТУ (QUIZ):
                1.  **СТРОГИЙ ЗАПРЕТ ПОДСКАЗОК:** Текст вопроса НИКОГДА не должен содержать ответ или намек на него.
                2.  **Проверка знаний:** Вопрос должен требовать знания конкретных фактов из презентации (цифры, сроки, условия), а не решаться логически.
                3.  **Структура:**
                    * Вопрос: Четкий, краткий.
                    * Варианты: 4 штуки, 1 верный, 3 дистрактора (неверных, но похожих на правду).
                    * Сгенерируй от 20 до 30 вопросов на основе слайдов.
