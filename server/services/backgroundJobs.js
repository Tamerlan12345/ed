...
`;
            ...
            finalPrompt = `
                ЗАДАНИЕ: Ты — Ведущий методолог страховой компании и эксперт по созданию образовательного контента.
                Твоя цель — создать МАКСИМАЛЬНО ПОДРОБНЫЙ, глубокий и исчерпывающий обучающий курс на основе ИСХОДНОГО ТЕКСТА.
                ...
                1.  **Максимальная детализация:** Не сокращай текст ради краткости. Твоя задача — не "сделать выжимку", а "научить всем нюансам". Если в тексте есть перечисления, условия, исключения — все они должны попасть на слайды.
                2.  **Полнота тем:** ИИ должен выявить все ключевые смысловые блоки текста и выделить под каждый из них отдельный слайд. Не смешивай разные сложные темы на одном слайде.
                3.  **Язык эксперта:** Используй профессиональную терминологию, но обязательно давай определения сложным понятиям.

                ПРАВИЛА РАБОТЫ С ЦИФРАМИ, ТАРИФАМИ И ПРИЛОЖЕНИЯМИ (Приоритет №1):
                1.  **Извлечение данных:** Если в тексте есть таблицы тарифов, страховые премии, коэффициенты или суммы — ТЫ ОБЯЗАН ИХ ПЕРЕНЕСТИ.
                2.  **Создание слайдов:** Под таблицы и цифры выделяй ОТДЕЛЬНЫЕ слайды (заголовок: "Тарифы и Условия"). Если таблица большая — разбей ее на 2-3 слайда.
                3.  **ВИЗУАЛИЗАЦИЯ ТАБЛИЦ (ВАЖНО):**
                    * Ты должен генерировать HTML-таблицы с ЯВНЫМИ ГРАНИЦАМИ.
                    * Используй строго этот формат: \`<table border="1" style="border-collapse: collapse; width: 100%; border: 1px solid black;">\`
                    * Для ячеек используй: \`<td style="border: 1px solid black; padding: 5px;">\` и \`<th style="border: 1px solid black; padding: 5px; background-color: #f0f0f0;">\`.
                    * Таблица не должна "висеть в воздухе", она должна быть четко очерчена линиями.

                СТРУКТУРА КУРСА (12-15 слайдов):
                * Слайд 1: Титульный (Название и цель курса).
                * Слайды 2-4: Основные понятия, объекты страхования, риски (Подробно!).
                * Слайды 5-8: Условия выплат, исключения, алгоритмы действий (Пошагово).
                * Слайды 9-10: **Блок Тарифов и Премий** (Таблицы с границами) - при условии что блок есть.
                * Слайды 11-14: Дополнительные условия, Практические примеры.
                * Последний слайд: Итоговое резюме.

                ФОРМАТ HTML:
                * Используй теги: \`<h3>\` (подзаголовки), \`<ul>\`/\`<ol>\` (списки), \`<strong>\` (акценты).
                * Разбивай "стены текста" на абзацы \`<p>\`.

                ТРЕБОВАНИЯ К ТЕСТУ (QUIZ):
                1.  Создай 20-30 вопросов, проверяющих знание деталей (цифр, сроков, конкретных условий из презентации).
                2.  Запрещены вопросы "на логику", только на знание фактов из материала.
