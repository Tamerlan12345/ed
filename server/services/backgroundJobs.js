...
`;
            ...
            finalPrompt = `
                ЗАДАНИЕ: Ты — Ведущий методолог образовательного центра.
                Твоя цель — создать МАКСИМАЛЬНО ПОДРОБНЫЙ, глубокий и структурированный обучающий курс на основе ИСХОДНОГО ТЕКСТА.
                ...
                СТРАТЕГИЯ КОНТЕНТА (Методология):
                1.  **Максимальная детализация:** Не сокращай текст ради краткости. Твоя задача — не "сделать выжимку", а "научить всем нюансам". Раскрывай тему полностью: если в тексте есть перечисления, условия, исключения, алгоритмы — все они должны попасть на слайды.
                2.  **Логика повествования:** Разбивай материал на логические блоки. Один сложный аспект — один слайд. Не смешивай все в кучу.
                3.  **Язык эксперта:** Используй профессиональную терминологию из текста, но давай пояснения. Тон должен быть обучающим и наставляющим.

                ПРАВИЛА РАБОТЫ С ЦИФРАМИ, ТАРИФАМИ И ТАБЛИЦАМИ (Приоритет №1):
                1.  **Поиск данных:** Если в тексте встречаются таблицы, тарифные сетки, страховые премии, лимиты, коэффициенты — ТЫ ОБЯЗАН ИХ ПЕРЕНЕСТИ в презентацию.
                2.  **Выделение места:** Под таблицы и цифры выделяй ОТДЕЛЬНЫЕ слайды. Если таблица большая — разбей ее на 2-3 слайда.
                3.  **ВИЗУАЛИЗАЦИЯ (ВАЖНО):**
                    * Генерируй HTML-таблицы ТОЛЬКО с явными границами.
                    * Формат: \`<table border="1" style="border-collapse: collapse; width: 100%; border: 1px solid black;">\`
                    * Ячейки: \`<td style="border: 1px solid black; padding: 5px;">\`, заголовок \`<th style="border: 1px solid black; padding: 5px; background-color: #f0f0f0;">\`.
                4.  **ОБЯЗАТЕЛЬНАЯ СНОСКА:** Сразу после любой таблицы с тарифами/деньгами добавь строку:
                    \`<br><small><em>* Информация справочная. Актуальные условия и тарифы проверяйте в официальной документации или программе.</em></small>\`

                СТРУКТУРА КУРСА (12-15 слайдов):
                * Слайд 1: Титульный (Название и цель курса).
                * Слайды 2-4: **Фундамент:** Основные понятия, определения, объекты и субъекты (Подробно!).
                * Слайды 5-8: **Процессы и Условия:** Как это работает? Алгоритмы, правила, исключения из правил (Пошагово).
                * Слайды 9-11: **Адаптивный блок:**
                    * *Если в тексте есть тарифы/цифры:* Вставь сюда таблицы с тарифами и премиями (с границами и сносками).
                    * *Если цифр нет:* Используй эти слайды для разбора сложных нюансов, тонкостей и специфических условий из текста.
                * Слайды 12-14: **Практика:** Примеры использования, частые ситуации, алгоритм действий сотрудника/клиента.
                * Последний слайд: Итоговое резюме (Check-list знаний).

                ФОРМАТ HTML:
                * Заголовки: \`<h3>\`.
                * Списки: \`<ul>\` (маркерированный), \`<ol>\` (нумерованный).
                * Акценты: \`<strong>\` для важного.
                * Абзацы: \`<p>\` для разделения мыслей.

                ТРЕБОВАНИЯ К ТЕСТУ (QUIZ):
                1.  Создай 20-30 вопросов.
                2.  Вопросы должны проверять знание ФАКТОВ из презентации (цифры, сроки, конкретные условия), а не общую логику.
                3.  Вопросы должны быть сложными и требовать внимательного изучения слайдов.

                ТРЕБОВАНИЯ К ВЫВОДУ (JSON):
                Верни СТРОГО валидный JSON. Структура: ${JSON.stringify(outputFormat)}
            `;
