...
const dialogueSimulator = async (req, res) => {
    ...
    try {
        ...
        // Общая инструкция по локализации, которую мы добавим во все промпты
        const localizationContext = `
            ВАЖНО - КОНТЕКСТ ЛОКАЛИЗАЦИИ (КАЗАХСТАН):
            1. Страна действия: Казахстан.
            2. Валюта: только Тенге. Никаких рублей.
            3. Термины:
               - Вместо "ОСАГО" используй "ОГПО" или "ОГПО ВТС".
               - Вместо "ПТС/СТС" используй "Техпаспорт" или "СРТС".
               - Вместо "ГИБДД" используй "Дорожная полиция" или "102".
            4. Законодательство и реалии должны соответствовать Казахстану.
        `;

        if (action === 'start') {
            const randomScenario = simulationScenarios[Math.floor(Math.random() * simulationScenarios.length)];

            // ОБНОВЛЕННЫЙ ПРОМПТ (START)
            const prompt = `
            Ты играешь роль клиента страховой компании.
            Твое настроение: "${persona}".
            Твоя конкретная ситуация (Сценарий): "${randomScenario}".

            ${localizationContext}

            ЗАДАНИЕ: Напиши ПЕРВОЕ сообщение в чат менеджеру.
            ТРЕБОВАНИЯ:
            1. Сообщение должно четко отражать твою ситуацию из сценария.
            2. Не пиши общие фразы типа "У меня вопрос". Сразу пиши суть проблемы (например: "Я попал в ДТП, что делать?" или "Хочу купить страховку на машину").
            3. Будь краток.
            `;
            const result = await model.generateContent(prompt);
            ...
            // Добавить сценарий в контекст чата
            ...
            // ОБНОВЛЕННЫЙ ПРОМПТ (CHAT)
            const prompt = `
            Ты — клиент страховой компании. Твоя ситуация: "${scenario}".
            Твое настроение: ${persona}.

            ${localizationContext}

            Твоя цель: Получить помощь по своей ситуации. Не отходи от сценария.
            Если менеджер спрашивает детали, которых нет в сценарии, придумай правдоподобный ответ, подходящий под контекст Казахстана (например, называй города Казахстана, если нужно).

            ИСТОРИЯ ДИАЛОГА:
            ${formattedHistory}

            Твой следующий ответ (коротко и по делу):`;
