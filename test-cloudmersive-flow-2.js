...
describe('Cloudmersive Integration Test', () => {
    ...
    beforeEach(() => {
        ...
        sinon.stub(supabase, 'createClient').returns(supabaseMock);
    ...
    it('should process a file successfully', async () => {
        ...
        assert(supabase.createClient().storage.from.calledWith('course-files'));
        assert(supabase.createClient().storage.from().getPublicUrl.calledWith('course-123/test.pdf'));
        assert(cloudmersiveMock.convertDocumentAutodetectToTxt.calledOnce);
        assert(supabase.createClient().from.calledWith('courses'));
        assert(supabase.createClient().from().update.calledWith({ source_text: 'This is the extracted text.', status: 'processed' }));
    });
});
